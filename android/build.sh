...
if [[ ${BUILD_AAR:-1} == 1 ]]; then
  ...
  cp "$root/android/aar/AndroidManifest.xml" "$root/android/aar/proguard.txt" "$aar_stage/"
  mkdir -p "$aar_stage/jni/arm64-v8a"
  cp "$out/package/jniLibs/arm64-v8a/"*.so "$aar_stage/jni/arm64-v8a/"
  (cd "$aar_stage" && zip -q -r seekdb-android.aar AndroidManifest.xml classes.jar proguard.txt jni)
...
if [[ ${BUILD_TEST_APK:-0} == 1 ]]; then
  ...
  "$bt/aapt2" link -o "$test_stage/unsigned.apk" --manifest "$root/android/test/AndroidManifest.xml" -I "$platform"
  javac --release 8 -cp "$platform:$out/package/seekdb-java.jar:$jdbc" -d "$test_stage/classes" \
  ...
  jar cf "$test_stage/test-classes.jar" -C "$test_stage/classes" .
  "$bt/d8" --min-api 28 --lib "$platform" --output "$test_stage/dex" \
    "$test_stage/test-classes.jar" "$out/package/seekdb-java.jar" "$jdbc"
  cp "$test_stage/dex/classes.dex" "$test_stage/classes.dex"
  cp "$out/package/jniLibs/arm64-v8a/"*.so "$test_stage/lib/arm64-v8a/"
  (cd "$test_stage" && zip -q -0 unsigned.apk classes.dex lib/arm64-v8a/*.so)
  "$bt/zipalign" -f -P 16 4 "$test_stage/unsigned.apk" "$test_stage/aligned.apk"
  ...
  "$bt/apksigner" sign --ks "$out/test/debug.jks" --ks-pass pass:android \
    --out "$out/package/seekdb-hybrid-test.apk" "$test_stage/aligned.apk"
